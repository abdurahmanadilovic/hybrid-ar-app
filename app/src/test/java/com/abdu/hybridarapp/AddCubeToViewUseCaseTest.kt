...
import com.abdu.hybridarapp.domain.AddCubeToViewImpl
import com.abdu.hybridarapp.domain.CreateAndAddCubeUseCase
...
class AddCubeToViewUseCaseTest {
    ...
    fun `when adding cube, should delegate to CreateAndAddCube use case`() = runBlocking {
        ...
        val expectedCube = DomainCube.empty()
...
private class MockCreateAndAddCubeUseCase(
    private val cubeToReturn: DomainCube
) : CreateAndAddCubeUseCase {
    var lastTapPosition: Position3d? = null
        private set

    override suspend fun invoke(tapLocation: Position3d): DomainCube {
